...
| ตอนได้ชื่อคลิป และก่อนเริ่มโหลด | มีไฟล์คลิปนี้ในโฟลเดอร์ปลายทางแล้ว | สถานะ **"มีแล้ว"** ไม่โหลด (ถ้าอยากโหลดซ้ำจริงๆ คลิกขวา → โหลดซ้ำ) |
...
**ประวัติการโหลด (`downloaded.txt`)** ชื่อไฟล์ไม่มีรหัสคลิปแล้ว แอปเลยให้ yt-dlp จดรหัสคลิปที่โหลดสำเร็จไว้ในไฟล์ `downloaded.txt` ข้างแอป
- **ล้างใหม่ทุกครั้งที่เปิดแอป** ประวัตินี้เลยกันซ้ำได้แค่ในรอบที่เปิดแอปอยู่ ข้ามรอบจะใช้การเช็คชื่อไฟล์ในโฟลเดอร์แทน
- ในรอบเดียวกัน โหลดคลิปเดิมซ้ำ (แม้จะลบไฟล์ไปแล้ว) จะขึ้น **"เคยโหลดแล้ว"** ไม่โหลดซ้ำ
- อยากโหลดใหม่จริงๆ: คลิกขวา → **โหลดซ้ำ** (ไม่เช็คประวัติ และเขียนทับไฟล์เดิม)
- ใช้กับเว็บที่ yt-dlp รองรับ ส่วนเว็บที่แอปอ่านหน้าเว็บเอง (missav, 7mmtv ฯลฯ) ใช้การเช็คชื่อไฟล์แทน
- โหลดซ้ำจะเขียนทับเฉพาะไฟล์ที่โหลดมาจากลิงก์เดียวกัน ไม่เขียนทับไฟล์ของคลิปอื่นที่ชื่อเหมือนกัน

## การตั้งค่า

| ตัวเลือก | ค่าเริ่มต้น | ความหมาย |
|---|---|---|
| รูปแบบ | อัตโนมัติ | **อัตโนมัติ** = ให้ yt-dlp เลือกให้ เอาแบบโหลดตรงก่อน (m3u8 ใช้เมื่อไม่มีทางเลือกอื่น) แล้วเอาตัวที่ชัดสุด / **MP4 ตรง** = บังคับไฟล์ mp4 ไฟล์เดียว ไม่ใช้ m3u8 / **ดีที่สุด** = เอาชัดสุดอย่างเดียว ไม่สนว่าเป็น m3u8 ไหม |
...
├── downloaded.txt    ประวัติรหัสคลิปที่โหลดในรอบนี้ ใช้กันโหลดซ้ำ (ล้างทุกครั้งที่เปิดแอป)
